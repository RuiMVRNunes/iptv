...
import ffmpegPathPkg from 'ffmpeg-static';const ffmpegPath = process.env.FFMPEG_PATH || ffmpegPathPkg;const __filename = fileURLToPath(import.meta.url);
const __dirname  = path.dirname(__filename);
const app = express();app.set('trust proxy', true);
app.get('/health', (_, res) => res.type('text').send('ok'));// CORS + noindex
...
const pickUA = req => UAS[String(req.query.ua||'').toLowerCase()] || UAS.vlc;const isM3U8 = (urlObj, headers) => {
...
function proxyOnce(targetUrl, req, res, hop = 0) {
  if (hop > 5) return !res.headersSent && res.status(508).send('Too many redirects');  let target;
  try { target = new URL(targetUrl); }
  catch { return !res.headersSent && res.status(400).send('invalid url'); }  const isHttps = target.protocol === 'https:', lib = isHttps ? https : http;  const headers = {
  ...
  if (req.query.origin)  headers['Origin']  = String(req.query.origin);  const options = {
    ...
    const status = up.statusCode || 0;// Redirect?
...
fs.mkdirSync(compatRoot, { recursive: true });function idFor(urlStr, mode, vbr, abr){
  ...
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });  const commonIn = [
    '-loglevel','warning',
    '-rw_timeout','15000000',
  ...
  }  const mapOut = ['-map','0:v:0?','-map','0:a:0?'];  const outArgs = ['-f','hls','-hls_time','4','-hls_list_size','6','-hls_flags','delete_segments+omit_endlist',
                   '-hls_segment_filename', path.join(dir, 'seg-%06d.ts'), path.join(dir, 'index.m3u8')];  let codecArgs;
  if (mode === 'remux'){
  ...
  }  const args = [...commonIn, '-i', srcUrl, ...mapOut, ...codecArgs, ...outArgs];  const proc = spawn(ffmpegPath, args, { stdio: ['ignore','pipe','pipe'] });
  const job = { id, dir, proc, last: Date.now(), mode, vbr, abr, srcUrl, failed:false };  proc.on('error', (err) => { job.failed = true; });
  proc.stdout.on('data', () => {});
  proc.stderr.on('data', () => {});
  proc.on('exit', (code) => {
  ...
  return job;
}function touchJob(id){ const j = jobs.get(id); if (j) j.last = Date.now(); }// Serve compat files and update last-access
app.use('/compat', (req, res, next) => {
  const m = req.path.match(/^\/([^/]+)/);
  if (m) touchJob(m[1]);
  next();
}, express.static(compatRoot, { maxAge: 0 }));// Endpoint to start (or reuse) a job, returns the play URL
app.get('/start-compat', async (req, res) => {
  if (!ffmpegPath) return res.status(500).json({ error:'ffmpeg binary not found' });  const raw = String(req.query.url || '');
  ...
  const origin = String(req.query.origin || '');  const id = idFor(raw, mode, vbr, abr);
  let job = jobs.get(id);
  ...
  const indexPath = path.join(compatRoot, job.id, 'index.m3u8');  if (wait){
...
}, 60*1000);const PORT = process.env.PORT || 8080;
app.listen(PORT, () => console.log('Server listening on http://0.0.0.0:' + PORT));package:
{
  "name": "iptv-proxy-render",
  "version": "1.2.1",
  "private": true,
  "type": "module",
  "scripts": {
